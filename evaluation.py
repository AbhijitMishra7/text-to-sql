...
def eval_sel(pred, label):
    ...
    cnt = 0 #Total matches of select columns and the aggregate functions with them
    cnt_wo_agg = 0 #Total matches of select columns without considering if aggregate functions match or not. eg=> select min(singer.age) would match with select singer.age 
...
def rebuild_condition_val(condition):
    #Rebuilds condition by removing the specific cell values not present in schema
    ...
    for idx, it in enumerate(condition):
        if idx % 2 == 0:
            ...
            # Directly add conditions which are just and/or
            res.append(it)
    return res


def rebuild_sql_val(sql):
    #Rebuilds sql by removing db specific cell values not present in schema
